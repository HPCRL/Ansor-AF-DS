...
conv_params = {}
for i, size in enumerate(sizesResnet):
...
def test_conv2d():
    for ite, key in enumerate(conv_params.keys()):
        ...
        log_file = "cuda_resnet_"+str(ite)+"_conv2d_N_"+str(N)+"_H_"+str(H)+"_W_"+str(W)+"_CO_"+str(CO)+"_CI_"+str(CI)+"_KH_"+str(KH)+"_KW_"+str(KW)+"_strides_"+str(strides)+"_padding_"+str(padding)+".json"
        ...
        print("cuda resnet: "+str(ite)+" conv2d for N = %d, H = %d, W = %d, CO = %d, CI = %d, KH = %d, KW = %d, strides = %s, padding = %s, correctness check passed!\n" % (N, H, W, CO, CI, KH, KW, strides, padding))
